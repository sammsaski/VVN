...
class Conv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=(1, 1), padding=(0, 0)):
        ...
        self.st_conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, padding=padding)
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, downsample=False):
        ...
        if self.downsample:
            self.downsampleconv = Conv(in_channels, out_channels, kernel_size=(1, 1), stride=(2, 2))
            self.downsamplebn = nn.BatchNorm2d(out_channels)
            self.conv1 = Conv(in_channels, out_channels, kernel_size, padding=padding, stride=(2, 2))
        else:
...
class ResLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, layer_size, block_type=ResBlock, downsample=False):
...
class R3D(L.LightningModule):
    ...
    def __init__(self, num_classes, layer_sizes, block_type=ResBlock):
        ...
        self.conv2 = ResLayer(64, 64, kernel_size=(3, 3), layer_size=layer_sizes[0], block_type=block_type)
        self.conv3 = ResLayer(64, 128, kernel_size=(3, 3), layer_size=layer_sizes[1], block_type=block_type, downsample=True)
        self.conv4 = ResLayer(128, 256, kernel_size=(3, 3), layer_size=layer_sizes[2], block_type=block_type, downsample=True)
        self.conv5 = ResLayer(256, 512, kernel_size=(3, 3), layer_size=layer_sizes[3], block_type=block_type, downsample=True)
