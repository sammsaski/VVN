...
%% Helper function
function ReachSets = NullFrameAttack(net, x)
    ReachSets = zeros(8, 1);

    for i = 1:8 % num of frames
        xTemp = x; % copy the original data
        xTemp(:, :, :, i) = zeros(32, 32); % nullify frame
        result = predict(net, xTemp); % get the output
        ReachSets(i) = result;
    end
end
