...
for ms = 1:length(models)
    ...
    for e = 1:nE
        ...
        disp("");
        save("results/ZoomIn/C3D_robustness_results_v2", "res", "time", "epsilon", "met");

    end
end

%% SINGLE PIXEL ALL FRAMES ROBUSTNESS VERIFICATION ON ZOOM OUT DATASET
...
for ms = 1:length(models)
    ...
    for e = 1:nE
        ...
        while any(classIndex ~= 10)
            ...
            iterationNum = sum(classIndex);
            fprintf('Iteration %d \n', iterationNum);
            [IS, xRand] = L_inf_attack_v2(datacopy(:,:,:,i), eps, nR); 
            t = tic;
            predictedLabels = predict(netonnx, xRand);
            [~, predictedLabels] = max(predictedLabels, [], 2);
            if any(predictedLabels-1 ~= labels(i))
        ...
        disp("Robust = "+string(sum(res(:,e)==1))+" out of " + string(N) + " videos");
        disp("");
        save("results/ZoomOut/C3D_robustness_results_SPAF_v1", "res", "time", "epsilon", "met");
...
for ms = 1:length(models)
    ...
    for e = 1:nE
        ...
        disp("Robust = "+string(sum(res(:,e)==1))+" out of " + string(N) + " videos");
        disp("");
        save("results/ZoomIn/C3D_robustness_results_SPAF_v1", "res", "time", "epsilon", "met");
...
%% SINGLE PIXEL, ALL FRAMES
function [IS, xRand] = L_inf_attack_v2(x, epsilon, nR)
    lb = x;
    ub = x;

    % Perturbation on all frames of the video
    for frameNum = 1:8
        lb(16, 16, frameNum) = x(16, 16, frameNum) - epsilon;
        if lb(16, 16, frameNum) < 0
            lb(16, 16, frameNum) = 0;
