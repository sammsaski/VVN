...
% Load the model
models = ["C3D_small_bcss_zoom_out_model_op11_v5.onnx",]; % "R3D_bcss_zoom_out_model_op11_v1.onnx"];
...
for ms = 1:length(models)
    ...
    for e = 1:nE
        ...
        disp("");
        save("results/SmallNetwork/ZoomOut/C3D_small_robustness_results_v5", "res", "time", "epsilon", "met");
...
% Load the model
models = ["C3D_small_bcss_zoom_in_model_op11_v4.onnx",]; % "R3D_bcss_zoom_in_model_op11_v1.onnx"];
...
for ms = 1:length(models)
    ...
    for e = 1:nE
        ...
        disp("Robust = "+string(sum(res(:,e)==1))+" out of " + string(N) + " videos");
        disp("");
        save("results/SmallNetwork/ZoomIn/C3D_small_robustness_results_v5", "res", "time", "epsilon", "met");
...
function [IS, xRand] = L_inf_attack(x, epsilon, nR, frameNum)
    vidSize = size(x);
    imgSize = vidSize(1);
    disturbance = zeros(imgSize) + epsilon;

    % disturbance = epsilon * ones(imgSize);

    lb = x;
    ub = x;

    lb(:, :, frameNum) = x(:, :, frameNum) - disturbance;
    
    if any(lb < 0)
        for i = 1:imgSize(1)
            for j = 1:imgSize(1)
                if lb(i, j, frameNum) < 0
                    lb(i, j, frameNum) = 0;
                end
            end
        end
   ...
   ub(:, :, frameNum) = x(:, :, frameNum) + disturbance;
    
    if any(ub > 0)
        for i = 1:imgSize(0)
            for j = 1:imgSize(0)
                if ub(i, j, frameNum) > 1
                    ub(i, j, frameNum) = 1;
                end
            end
        end
